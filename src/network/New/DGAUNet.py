...
class Encoder(nn.Module):
    def __init__(self,img_ch=3,output_ch=1,vis=False, img_size=None):
        super(Encoder,self).__init__()
        ...
        self.position_embeddings = nn.Parameter(torch.zeros(1, self.img_size, 768))
        self.dropout = Dropout(0.1)
        self.transformer=Transformer(vis)
    ...
    def forward(self,x):
        ...
        x6 = x6.flatten(2) # (B*768*196)
        x6 = x6.transpose(-1, -2)  #  (B*196*768)
        position_embeddings = nn.Parameter(torch.zeros(1, x6.shape[1], 768)).to(x6.device)
        x7 = x6 + position_embeddings
        x8 = self.dropout(x7)
        x9, attn_weights = self.transformer(x8)
        B, n_patch, hidden = x9.size()  # reshape from (B, n_patch, hidden) to (B, h, w, hidden)
        h, w = int(np.sqrt(n_patch)), int(np.sqrt(n_patch))
...
class DGAUNet(nn.Module):
    def __init__(self, img_ch=3, output_ch=1,vis=False, img_size = None):
        super(DGAUNet, self).__init__()
        self.img_size = img_size
        self.encoder=Encoder(img_ch,output_ch,vis, self.img_size)
        self.encoder_label=Encoder(img_ch,output_ch,vis,self.img_size)
        self.decoder=Decoder(output_ch)
