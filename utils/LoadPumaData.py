...
class PumaTissueDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if isinstance(self.imgs[idx], str):
            image = np.load(self.imgs[idx].replace('.tif','.npy')).astype("float32")
            temp = np.load(self.masks[idx].replace('.tif','.npy')).astype("int16")

            # temp = dilate_erode(temp, itersations=1, dilate=False)
            a = np.copy(temp)
            if np.sum(temp == 3):
                a[a != 3] = 0
                a = dilate_erode(a, itersations=2, dilate=False)
                temp[temp == 3] = 0
                temp[a == 3] = a[a == 3]
            if image.shape[0]*image.shape[1] > 128*128:
                im = cv2.cvtColor(cv2.imread(self.imgs[idx].replace('/images/', '/ourMethodResults/')),
                                  cv2.COLOR_BGR2GRAY) * 50
            else:
                im = np.ones((temp.shape[0], temp.shape[1]))*50*3.5
